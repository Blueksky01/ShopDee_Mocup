...
<div class="max-w-4xl mx-auto my-6 bg-white rounded-[32px] shadow-2xl border border-terracotta/20 overflow-hidden grid grid-cols-1 md:grid-cols-12">
    ...
    <!-- LEFT SIDE: Visual Showcase (Noir Black Theme) -->
    <div class="md:col-span-5 bg-gradient-to-br from-[#2d3748] via-[#1a202c] to-[#0d1117] text-white p-8 flex flex-col justify-between relative overflow-hidden">
        ...
        <!-- Member Privileges Showcase (Without Jacket Image) -->
        <div class="relative z-10 my-8 space-y-4 text-xs text-gray-200">
            <div class="flex items-center space-x-3 bg-white/10 backdrop-blur p-3 rounded-2xl border border-white/10">
                ...
                <div>
                    <p class="font-bold text-white">สมาชิกสิทธิพิเศษ</p>
                    <p class="text-[11px] text-gray-300">รับโปรโมชันพิเศษเฉพาะผู้ใช้งานสมาชิก</p>
                </div>
            </div>

            <div class="flex items-center space-x-3 bg-white/10 backdrop-blur p-3 rounded-2xl border border-white/10">
                <div class="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-sm text-green-400">
                    <i class="fa-solid fa-clock-rotate-left"></i>
                </div>
                <div>
                    <p class="font-bold text-white">บันทึกประวัติออเดอร์</p>
                    <p class="text-[11px] text-gray-300">ตรวจสอบสถานะคำสั่งซื้อย้อนหลังได้ตลอดเวลา</p>
                </div>
            </div>

            <div class="flex items-center space-x-3 bg-white/10 backdrop-blur p-3 rounded-2xl border border-white/10">
                <div class="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-sm text-sky-400">
                    <i class="fa-solid fa-bolt"></i>
                </div>
                <div>
                    <p class="font-bold text-white">สั่งซื้อรวดเร็ว</p>
                    <p class="text-[11px] text-gray-300">บันทึกที่อยู่จัดส่งและขั้นตอน Checkout อัตโนมัติ</p>
                </div>
            </div>
        </div>
    ...
    <!-- RIGHT SIDE: Interactive Register Form & Social Register -->
    <div class="md:col-span-7 p-8 sm:p-10 flex flex-col justify-between space-y-6">
        <div>
            ...
            <p class="text-xs text-rust/60 mt-1">กรอกข้อมูลส่วนตัวเพื่อสร้างบัญชีสมาชิก หรือสมัครผ่าน Social</p>

            <!-- Register Form -->
            <form method="POST" action="{{ route('register') }}" class="space-y-3.5 mt-5">
                @csrf
            ...
            <!-- Social Register Mockup Section -->
            <div class="mt-5 space-y-3">
                <div class="relative flex items-center justify-center">
                    <div class="border-t border-terracotta/20 w-full"></div>
                    <span class="bg-white px-3 text-[11px] font-bold text-rust/60 uppercase tracking-wider relative">หรือ สมัครด้วย Social</span>
                </div>

                <div class="grid grid-cols-3 gap-2.5">
                    <!-- Google Mockup Button -->
                    <button type="button" onclick="alert('Mockup Social Register: สมัครสมาชิกด้วย Google สำเร็จ!')"
                    ...
                    <button type="button" onclick="alert('Mockup Social Register: สมัครสมาชิกด้วย Facebook สำเร็จ!')"
                    ...
                    <!-- Apple Mockup Button -->
                    <button type="button" onclick="alert('Mockup Social Register: สมัครสมาชิกด้วย Apple ID สำเร็จ!')"
